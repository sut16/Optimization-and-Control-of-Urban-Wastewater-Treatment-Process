...
%load("C:\Users\匡\Desktop\污水处理\BSM11\NEWMOEADARP_DRY");
% clear all
%  load("C:\Users\匡\Desktop\污水处理\BSM11\optimaData\NSGAIII_Optimal_2500FT_data.mat");
...
inpart=in(startindex:(stopindex-1),:);  %waste water in plant
...
Qevec=settlerpart(:,31).*timevector;%出水流量
Qinvec=inpart(:,15).*timevector;%入水流量
%%%%%%全部出水组分质量
...
Qetot = sum(Qevec); %all of qe
Qeav = Qetot/totalt;%每天的平均出水流量
...
kla1vec = kla1in*ones((stopindex)-startindex,1);
kla2vec = kla2in*ones((stopindex)-startindex,1);
kla3vec = kla3in*ones((stopindex)-startindex,1);
kla4vec = kla4in*ones((stopindex)-startindex,1);
...
carbon1vec = carbon1in*ones((stopindex)-startindex,1);
carbon2vec = carbon2in*ones((stopindex)-startindex,1);
carbon3vec = carbon3in*ones((stopindex)-startindex,1);
carbon4vec = carbon4in*ones((stopindex)-startindex,1);
carbon5vec = carbon5in*ones((stopindex)-startindex,1);
...
% 
% figure(17)
% plot(time(1:(end-1))-starttime,BOD5in, 'k')
% %hold on 
% %plot(time,kla5inpart, 'b')
% xlabel('时间 (天)')
% ylabel('BOD组分(m^3/d)')
...
% 
% figure(18)
% plot(time(1:(end-1))-starttime,CODin, 'k')
% %hold on 
% %plot(time,kla5inpart, 'b')
% xlabel('时间 (天)')
% ylabel('COD组分(m^3/d)')
